        ...
        @foreach($teams as $team)
          <a href="/teams/{{ $team['id']}}" class="group relative">
            <div class="pb-20">
              ...
              <div class="grid grid-cols-2 max-h-[70px] min-w-[586px] absolute top-[137px] border-l-2 border-t-2 border-black group-hover:border-{{ $team->colour }} rounded-tl-lg">
                <div>  
                  <h3 class="pt-4 pl-2 text-lg font-medium text-gray-900 group-hover:text-{{ $team->colour }}">{{ $team->team_name }}</h3>
                  <p class="mt-1 pl-2 text-sm text-gray-900 group-hover:text-{{ $team->colour }}">{{ $team->drivers }}</p>
                </div>
